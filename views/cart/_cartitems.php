...
<div id="genericcart">
    <div class="row-fluid main-cart-header">
        <div class="span3"><span class="cart_header"><?= Yii::t('cart','Product'); ?></span></div>
		...
		<?php foreach($model->cartItems as $item): ?>
			<div class="row-fluid cart-list-item">
			    ...
			    <div class="span3 centeritem cart-item-quantity"><span class="cart_qty"><?php
		...
		<div class="cart-summary">
			<div class="row-fluid remove-bottom"><span class="span4 offset8 border-top"></span></div>
	...
	<?php if (isset($this->intEditMode) && $this->intEditMode): ?>
		<div class="row-fluid">
			<div class="span12 errorMessage cart-directions">
...
$(document).ready(function() {
	$('body').addClass('edit-cart');
	
	if($(window).width() < 767) {
		$('#ShoppingCart div.update').hide();
	}
	$('.cart-list-item').append("<a class='cart-item-remove fa fa-remove'></a>");
	$('div.cart-directions').hide();
	$('.cart-item-remove').click(function (e) {
		e.preventDefault();

		var answer = confirm('Remove item ' + $(this).siblings('.span3').children('a').text() + " from cart?");
		if (answer) {
			console.log('yes'); 
			console.log($(this).siblings('.cart-item-quantity').children().children('input').val(0));
